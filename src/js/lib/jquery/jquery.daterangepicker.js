	...
	$.fn.dateRangePicker = function(opt)
	{
		...
		function calcPosition()
		{
			if (!opt.inline)
			{
				var offset = $(self).offset();
						...
						{
							box.css({
								top: 4 + offset.top+$(self).outerHeight() + parseInt($('body').css('border-top') || 0,10 ),
								left: offset.left
							});
						}
			}
		...
		function createDom()
		{
			...
			if (opt.showTopbar)
			{
				...
				html += '<button class="ui red mini circular basic disabled icon apply-btn button"><i class="remove icon"></i></button>';
				html += '</div>'
			}

			var _colspan = opt.showWeekNumbers ? 6 : 5;
			html += '<div class="month-wrapper">'
				+'<table class="month1" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;"><span class="prev"><i class="chevron left icon"></i></span></th><th colspan="'+_colspan+'" class="month-name"></th><th style="width:27px;">' + (opt.singleDate || !opt.stickyMonths ? '<span class="next"><i class="chevron right icon"></i></span>': '') + '</th></tr><tr class="week-name">'+getWeekHead()+'</thead><tbody></tbody></table>';

			if ( hasMonth2() )
			{
				html += '<div class="gap">'+getGapHTML()+'</div>'
					+'<table class="month2" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;">' + (!opt.stickyMonths ? '<span class="prev"><i class="chevron left icon"></i></span>': '') + '</th><th colspan="'+_colspan+'" class="month-name"></th><th style="width:27px;"><span class="next"><i class="chevron right icon"></i></span></th></tr><tr class="week-name">'+getWeekHead()+'</thead><tbody></tbody></table>'
			}
				//+'</div>'
